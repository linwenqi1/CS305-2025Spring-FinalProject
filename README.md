...
* Create a TCP socket and bind it to the peer鈥檚 IP address and port.
...
* Define the JSON format of a `hello` message, which should include: `{message type, sender鈥檚 ID, IP address, port, flags, and message ID}`. A `sender鈥檚 ID` can be `peer_port`. The `flags` should indicate whether the peer is `NATed or non-NATed`, and `full or lightweight`. The `message ID` can be a random number.
